...
public class GameGUI extends Application {
	...
	private Parent createContent() {
		...
		root.getChildren().addAll(makeColumns(Observer.o.getSize(), board)); // Draws
																				// the
																				// data
																				// in
																				// the
	...
	private Shape makeGrid(int size) {
		// makes the grid
		Shape shape = new Rectangle((size + 1) * 80, (size + 1) * 80); // Creates
																		// the
																		// screen
																		// of
																		// the
		...
		for (int i = 0; i < size; i++) // For each row
		{
			...
			{
				// Creates the circle
				Circle circle = new Circle(Observer.o.getPieceSize() / 2, Observer.o.getPieceSize() / 2,
						Observer.o.getPieceSize() / 2);
	...
	private List<Rectangle> makeColumns(int size, BoardPiece[][] board) {
		...
		for (int i = 0; i < size; i++) {
			RectangleSubclass rect = new RectangleSubclass(Observer.o.getPieceSize(),
					(size + 1) * Observer.o.getPieceSize(), i);
